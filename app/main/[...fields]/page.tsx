...
const DynamicPage = ({ params, searchParams }: any) => {
  ...
  if (fieldId && !["edit", "delete"].includes(action)) {
    ...
    return (
      <div>
        {action === "create" && <CreateField model={allModels.find((m) => m.model === model)} page={true} />}
    ...
    return (
      <div>
        {model === "blog" && <ListModelData modelSlug={model} />}
        {model === "comment" && <CommentBox modelSlug={model} />}
      </div>
    );
  }
};
